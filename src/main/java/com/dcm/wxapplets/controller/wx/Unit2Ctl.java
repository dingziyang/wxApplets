package com.dcm.wxapplets.controller.wx;
...
import com.dcm.wxapplets.service.UnitService;
...
@Api(value = "/wx/unit", description = "单位接口")
@RestController
@RequestMapping("/wx/unit")
public class Unit2Ctl {
...
//        vo.setData(children);
    ...
    public ResultVO<Integer> create(@RequestBody @Valid Unit unit, BindingResult validResult, HttpServletRequest request){
        ...
        if (CollectionUtils.isEmpty(existUnits)) {
            ...
            throw new DataExistedException("菜单编码["+ unit.getCode() +"]已存在，不可重复创建");
        }
        return vo;
    }
    ...
    public ResultVO<Integer> update(@RequestBody @Valid Unit unit, BindingResult validResult, HttpServletRequest request){
        ...
        if(unit.getId() == null) {
            throw new FormValidException("缺少主键：id");
        }
        ...
        if (CollectionUtils.isEmpty(existUnits)) { // 不可重复项都不匹配
            ...
            if (existUnits.size() == 1) { // 找到一个匹配项
                if(existUnits.get(0).getId().equals(unit.getId())) { // 是自己，则修改
                    ...
                    throw new DataExistedException("菜单编码["+ unit.getCode() +"]已存在，不可修改为此值");
                }
            } else { // 找到多个匹配项，则表示已存在
                throw new DataExistedException("菜单编码["+ unit.getCode() +"]已存在，不可修改为此值");
            }
    ...
    /**
      ...
      * @param: ids
      * @return com.sdcm.common.constant.ResultVO<java.lang.Integer>
      * @throws
      */
    ...
    @GetMapping("/unitForm")
    public ModelAndView geUnitFormPage(){
        ModelAndView modelAndView = new ModelAndView("/system/jbsz/unit/unitForm");
        return modelAndView;
    }
}
